...
const NicuPhotos = ({ data, field }: { data: string[]; field: string }) => {
  ...
  return (
    <>
          ...
          data?.map((img: string) => (
            ...
            >
              <Image
                ...
                preview={false}
              />
              <span className="w-full h-full bg-black/40 backdrop-blur-sm absolute top-0 left-0 opacity-0 group-hover:opacity-100 duration-200"></span>
              <button
                onClick={() => handleDeleteImage(img)}
                className="hidden group-hover:inline-block transition-all duration-200 absolute top-2 left-2 px-1 py-1.5 rounded-md bg-white text-secondary"
              >
      ...
      <div className="border rounded-xl mt-5 px-5 py-6">
        <p className="font-medium mb-1.5">Add NICU Photos</p>
